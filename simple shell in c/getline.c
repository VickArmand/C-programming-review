...
int main()
{
    ...
    // if *lineptr is set to null and n is et to 0 then getline() allocates buffer for storing the line. The buffer should be freed by user program even if getline() failed
    printf("%s\n", lineptr);
    free(lineptr);
    return (0);
}